...
import com.stuart.stuartapp.StuartApplication;
...
public class ToastUtil {
    ...
    public static void showMessage(final CharSequence msg, final int len) {
        if (msg == null || msg.equals("")) {
            LogUtil.i(TAG, "showMessage", "[ToastUtil] response message is null.");
            return;
        }
            ...
            public void run() {
                synchronized (synObj) { //加上同步是为了每个toast只要有机会显示出来
                    if (toast != null) {
                        ...
                        toast = Toast.makeText(StuartApplication.getInstance().getApplicationContext(), msg, len);
                    }
                    toast.show();
                }
            }
