...
import com.stuart.stuartapp.utils.Randoms;
              ...
              //  .flatMap(flow -> Observable.range(0, mEmojiPer))
                .flatMap(new Func1<Long, Observable<?>>() {
                    public Observable<?> call(Long aLong) {
                        Log.i("stuart", "call1 " + aLong);
                        return Observable.range(0,mEmojiPer);
                    }
                })
            //    .map(i -> mEmojiPool.acquire())
              .map(new Func1<Object, ImageView>() {
                  public ImageView call(Object xx) {
                      Log.i("stuart", "call2 " + xx);
                      return mEmojiPool.acquire();
                  }
              })
//                .filter(e -> e != null)
                .filter(new Func1<ImageView, Boolean>() {
                    @Override
...
//                .subscribe(this::startDropAnimationForSingleEmoji, Throwable::printStackTrace);
        .subscribe(new Action1<ImageView>() {
            ...
            public void call(ImageView o) {
                Log.i("stuart", "call3 " + o);
                startDropAnimationForSingleEmoji(o);
            }
        }, new Action1<Throwable>() {
            @Override
            public void call(Throwable throwable) {
                throwable.printStackTrace();
                Log.i("stuart", "call3 " + throwable.getLocalizedMessage());
            }
        });
